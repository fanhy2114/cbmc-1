  ...
  const goto_functionst &goto_functions)
{
  goto_functionst::function_mapt::const_iterator f_it =
    goto_functions.function_map.find(node.function);
      ...
      ) // no ++it
  {
    ...
    else
    {
          ...
          ++d_it)
      {
        ...
        if(cfg[entry->second].node_required)
        {
          const irep_idt &id2 = cfg[entry->second].function;
          INVARIANT(id==id2,
  ...
  const slicing_criteriont &criterion)
{
  ...
  forall_goto_functions(f_it, goto_functions)
  {
    forall_goto_program_instructions(i_it, f_it->second.body)
      cfg[cfg.entry_map[i_it]].function = f_it->first;
  }
  ...
  for(cfgt::entry_mapt::iterator
      ...
      e_it++)
  {
    if(criterion(cfg[e_it->second].function, e_it->first))
